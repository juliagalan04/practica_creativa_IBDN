...
object MakePrediction {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // define a streaming query para guardar en MongoDB
    val mongoStreamWriter = finalPredictions
      .writeStream
      .format("mongodb")
    ...
    val hdfsStreamWriter = finalPredictions
      .writeStream
      .format("parquet")  // puedes usar también "json" o "csv"
      .option("path", "hdfs://namenode:9000/user/spark/output")
      .option("checkpointLocation", "/tmp/hdfs_checkpoint")
      .outputMode("append")
      .start()


    // define otra streaming query para escribir en Kafka
    ...
    // Esperar a que terminen los tres flujos de salida
    mongoStreamWriter.awaitTermination()
    hdfsStreamWriter.awaitTermination()
